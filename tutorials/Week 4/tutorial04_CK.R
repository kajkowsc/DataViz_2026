...
begin_plot <- ggplot(data = co2_rankings, aes(x = rank_1995, y = rank_2014)) +
  ...
  annotate(geom = "text", x = 40, y = 6, label = "Good Boys",
           hjust = 0, vject = 1, color = "#2ECC40") +
  annotate(geom = "text", x = 162.5, y = 135, label = "Bad\nBoys",
           hjust = 0.5, vject = 1, color = "#FF851B") +
  annotate(geom = "segment", x = 38, xend = 20, y = 6, yend = 6, color = "#2ECC40",
           arrow = arrow(angle = 25, length = unit(0.5, "lines"))) + 
  annotate(geom = "segment", x = 162.5, xend = 162.5, y = 140, yend = 155, color = "#FF851B",
           arrow = arrow(angle = 25, length = unit(0.5, "lines"))) +
  scale_color_manual(values = c("grey50", "pink", "lightblue")) +
