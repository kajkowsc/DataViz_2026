...
judicial_review <- judicial_review %>%
  select(CASE, DATE, YEAR, EFFECT, S.DATE, TIME.TO.DECISION, CONGRESS, AREA1, ATT.GEN, REVERSD, DISSENT) %>%
  filter(YEAR >= 1900)
...
table(judicial_review$AREA1) #1 Due Process
#or 
judicial_review %>% 
...
cases_by_year <- judicial_review %>%
  count(YEAR)
p1 <- ggplot(cases_by_year, aes(x = YEAR, y = n)) +
  geom_line()

invalid_share <- judicial_review %>%
  group_by(YEAR) %>%
  summarize(
    share_invalid = mean(EFFECT != "upheld", na.rm = TRUE)
  )
p2 <- ggplot(invalid_share, aes(x = YEAR, y = n) +
               geom_line()
...
model_jr <- lm(data = judicial_review, TIME.TO.DECISION ~ EFFECT + CONGRESS + ATT.GEN + DISSENT)
